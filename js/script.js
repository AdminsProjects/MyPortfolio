...
hBody = $("#header-home").height(),
wBody = $("#header-home").width();
	$('.flotate').each(function(i){
		t = "translate("+aNumero(-100, wBody + 50 )+"px,"+aNumero(-100, hBody + 50)+"px)";
		$(this).css("transform",t);
	});
moverF();

setTimeout(function(){
		$(".loading.filter").css("opacity",0);
	setTimeout(function() {
	...
	$('.flotate').css("transition", "transform 7s linear");
...
function moverF(){
	...
	$('.flotate').each(function(i){
		...
		t = "translate("+aNumero(-100, wBody + 50 )+"px,"+aNumero(-100, hBody + 50)+"px)";
		if (b) {
			$(this).css({"transform":t,"opacity":.2});
		} else {
			$(this).css({"transform":t,"opacity":0});
		}
	});
		moverF();
	},6500);
...
window.onscroll = () =>{
    	var s = getScroll(),
    	headerHome = $("#header-home").height() + $("#section-1").height() - 10,
    	myEnvironment = headerHome + $("#my-environment").height();
    	skills =  myEnvironment + $("#skills").height();
    	project = skills + $("#project").height();
    	contact = project  + $("#contact").height();

	    $('nav ol li a').removeClass('active');

	    if (s<headerHome){
	    	$('#go-header-home').addClass('active');
	    }if (s>headerHome&&s<myEnvironment){
	    	$('#go-my-environment').addClass('active');
	    }if (s>myEnvironment&&s<skills){
	    	$('#go-skills').addClass('active');
	    }if (s>skills&&s<project){
	    	$('#go-project').addClass('active');
	    }if (s>project&&s<contact){
	    	$('#go-contact').addClass('active');
	    } 
    };
